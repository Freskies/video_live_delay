...
impl DelayApp {
	...
	fn restart_pipeline(&mut self) {
		...
		// Pipeline accelerata via GPU a 60 FPS stabili (zero-copy CPU)
...
fn main() {
	...
	let app_state = Rc::new(RefCell::new(DelayApp::new()));
	app_state.borrow_mut().restart_pipeline();

	// Finestra di controllo Touch OSD (compatta, sempre sopra e a misura corretta)
	let window = gtk::Window::new(gtk::WindowType::Toplevel);
	window.set_title("Video Delay Control");
	...
	btn_minus.set_size_request(130, 50);

	let lbl_status = gtk::Label::new(None);
	lbl_status.set_markup("<span font='17' weight='bold'>Delay: 3s | 0°</span>");
	lbl_status.set_hexpand(true);

	let btn_plus = gtk::Button::with_label("➕  +1s");
	btn_plus.set_size_request(130, 50);
	...
	// Gestione touch Meno
	{
		...
		btn_minus.connect_clicked(move |_| {
			...
			if app.delay_sec > 1 {
				app.delay_sec -= 1;
				lbl.set_markup(&format!("<span font='17' weight='bold'>Delay: {}s | {}°</span>", app.delay_sec, app.rotation_deg));
