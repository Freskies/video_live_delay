...
impl DelayApp {
	fn new(sink: gstreamer::Element) -> Self {
		Self {
			pipeline: None,
			sink,
			delay_sec: 3,
	...
	fn restart_pipeline(&mut self) {
		...
		pipe.add_many([&src, &filter1, &flip, &queue, &conv, &self.sink]).unwrap();
		gstreamer::Element::link_many([&src, &filter1, &flip, &queue, &conv, &self.sink]).unwrap();
